...
Locate the SD card on your current system:
```sh
lsblk
```
...
Format the disk (where `/dev/sdx` is the disk you found):
```sh
wipefs -a /dev/sdx
```
...
```sh
mkfs.ext4 /dev/sdx2  # Optional but useful if we want to use lbu to store persistent changes
```

```sh
mkdir -p /tmp/alpineusb
```

```sh
mount /dev/sdx1 /tmp/alpineusb
```
