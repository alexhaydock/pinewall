...
x86:
	...
	podman run --rm -it -v "$(shell pwd)/images:/tmp/images:Z" --tmpfs "/tmp/cache" pinewall-x86
...
rpi:
	...
	podman run --rm -it -v "$(shell pwd)/images:/tmp/images:Z" --tmpfs "/tmp/cache" pinewall-rpi
